...
const TARGETS = ["pe","it","us","ye","X"];
function findFn(name: string): string[] {
  const out: string[] = [];
  function visit(n: AstNode): void {
    if (n.type === "FunctionDeclaration" && String((n.id as AstNode)?.name ?? "") === name) out.push(code.slice(n.start ?? 0, Math.min((n.start ?? 0)+260, n.end ?? 0)).replace(/\n/g," "));
    if (n.type === "VariableDeclarator" && String((n.id as AstNode)?.name ?? "") === name) {
      const init = n.init as AstNode | undefined;
      if (init && (init.type === "ArrowFunctionExpression" || init.type === "FunctionExpression")) out.push(code.slice(init.start ?? 0, Math.min((init.start ?? 0)+260, init.end ?? 0)).replace(/\n/g," "));
    }
...
for (const t of TARGETS) {
  console.log(`### ${t}`);
  for (const d of findFn(t).slice(0,2)) console.log(`   ${d.slice(0,200)}`);
}
