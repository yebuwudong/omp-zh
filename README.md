...
把已安装的官方 omp `v18.2.1` 界面汉化为简体中文，**不改变版本、不替换安装、不动运行时结构**。
...
- 翻译来自社区分支 `yequ172672/oh-my-pi-cn`（基线 17.4.1），个别术语与 18.2.1 新功能不完全对齐。
...
这里复刻了上游汉化分支的 `valueLabels` 机制（该分支在组件层实现；18.2.1 产物还没有这个字段，
所以在字符串层等价重建）：
