...
把已安装的官方 omp `v18.2.2` 界面汉化为简体中文，**不改变版本、不替换安装、不动运行时结构**。
...
补丁器针对 18.x 产物结构编写。跨大版本升级（如 19.x）后建议先 `./omp-zh.sh report` 看
命中数量是否仍在 2000 上下；若骤降说明上游改了产物结构，需要先更新补丁规则。
...
- 翻译来自社区分支 `yequ172672/oh-my-pi-cn`（基线 17.4.1），个别术语与 18.2.2 新功能不完全对齐。
...
这里复刻了上游汉化分支的 `valueLabels` 机制（该分支在组件层实现；18.2.2 产物还没有这个字段，
所以在字符串层等价重建）：
